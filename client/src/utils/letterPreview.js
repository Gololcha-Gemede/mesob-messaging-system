...
function header({ templateLabel, recipientName }) {
  return `
    ...
    <section class="letter-meta-grid">
      ...
      <div><strong>From:</strong> Current user</div>
      <div><strong>To:</strong> ${escapeHtml(recipientName || 'Recipient')}</div>
    </section>
  `;
}

export function buildClientLetterPreview({ templateType, recipientName, subject, content, file }) {
  ...
  if (template.value === 'memo') {
    body = `
      ${header({ templateLabel: 'Memo', recipientName })}
    ...
    body = `
      ${header({ templateLabel: 'Notice', recipientName })}
    ...
    body = `
      ${header({ templateLabel: 'Circular', recipientName })}
    ...
    body = `
      ${header({ templateLabel: 'Request Form', recipientName })}
    ...
    body = `
      ${header({ templateLabel: 'Official', recipientName })}
      <section class="letter-subject"><span>Subject</span><strong>${safeSubject}</strong></section>
  ...
  return `
    <article class="official-letter" data-template="${escapeHtml(template.value)}">
      ...
      <section class="letter-signature"><strong>Current user</strong></section>
